...
class FilterHooks {
	...
	private function __construct() {
		...
		$this->loader->add_filter( 'woocommerce_product_get_price', $this, 'cptwoo_product_get_price', 15, 2 );
		...
		$this->loader->add_filter( 'woocommerce_checkout_create_order_line_item_object', $this, 'checkout_create_order_line_item_object', 12 );
	...
	public function cptwoo_product_get_price( $price, $product ) {
		if ( ! apply_filters( 'cptwoo_product_is_set_price', true, $product, $price ) ) {
			return $price;
		}
		$post_type = get_post_type( $product->get_id() );
		if ( ! Fns::is_supported( $post_type ) ) {
