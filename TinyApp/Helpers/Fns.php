...
class Fns {
	...
	public static function get_options() {
		...
		if ( ! empty( $options['selected_post_types'] ) ) {
			$options['selected_post_types'] = array_filter(
		...
		self::$cache[ $cache_key ] = apply_filters( 'cptwooint_default_settings', $options );
		return self::$cache[ $cache_key ];
	}
	 ...
	 * @return bool
	 */
	public static function is_enabled( $post_type, $key ) {
	...
	public static function is_archive_page_like_shop_page( $post_type ) {
		return self::is_enabled( $post_type, 'archive_similar_shop_page' );
	}
	...
	public static function is_single_page_like_product_page( $post_type ) {
		return self::is_enabled( $post_type, 'details_similar_product_page' );
	}
	...
	public static function is_review_enabled( $post_type ) {
		return self::is_enabled( $post_type, 'enable_product_review' );
	}
	...
	public static function is_schema_enabled( $post_type ) {
		return self::is_enabled( $post_type, 'enable_product_schema' );
	}
	...
	public static function pro_feature_list() {
		return [
			...
			[
				'title' => 'Integrated Post Display on the Shop Page',
				'desc'  => 'The integrated post will be displayed on the shop page, providing customers with relevant content and updates directly alongside the products, enhancing the shopping experience with valuable information and insights.',
			],
			[
